...
type Category = "brokers" | "crypto" | "payments" | "cards";
...
export default function ComparePage() {
  ...
  // Select defaults on category change
  useEffect(() => {
    if (category === "brokers") {
      setItem1Id(BROKERS_DATA[0]?.slug || "");
      setItem2Id(BROKERS_DATA[1]?.slug || "");
    } else if (category === "crypto") {
      setItem1Id(CRYPTO_APPS_DATA[0]?.slug || "");
      setItem2Id(CRYPTO_APPS_DATA[1]?.slug || "");
    } else if (category === "payments") {
      setItem1Id(PAYMENT_APPS_DATA[0]?.slug || "");
      setItem2Id(PAYMENT_APPS_DATA[1]?.slug || "");
    } else if (category === "cards") {
      setItem1Id(CREDIT_CARDS_CATALOG[0]?.slug || "");
      setItem2Id(CREDIT_CARDS_CATALOG[1]?.slug || "");
    }
  }, [category]);
  ...
  // Get option lists
  const getItemList = () => {
    if (category === "brokers") return BROKERS_DATA.map((x) => ({ slug: x.slug, name: `${x.name} (${x.country})` }));
    if (category === "crypto") return CRYPTO_APPS_DATA.map((x) => ({ slug: x.slug, name: `${x.name} (${x.country})` }));
    if (category === "payments") return PAYMENT_APPS_DATA.map((x) => ({ slug: x.slug, name: `${x.name} (${x.country})` }));
    if (category === "cards") return CREDIT_CARDS_CATALOG.map((x) => ({ slug: x.slug, name: `${x.name} (${x.country})` }));
    return [];
  };
    ...
    <div className="app-container" style={{ paddingTop: "2.5rem" }}>
      ...
      <header style={{ marginBottom: "2.5rem", textAlign: "center" }}>
        ...
        <p style={{ color: "var(--text-secondary)", marginTop: "0.5rem", fontSize: "1.05rem" }}>
          Select a category and compare two items side-by-side to review fees, limits, checklists, and pros/cons.
        </p>
      </header>

      {/* Category selector */}
      <div style={{ display: "flex", justifyContent: "center", gap: "1rem", flexWrap: "wrap", marginBottom: "3rem" }}>
        {(["brokers", "crypto", "payments", "cards"] as Category[]).map((cat) => {
          const isActive = category === cat;
          const label = cat === "brokers" ? "💼 Stock Brokers" : cat === "crypto" ? "🪙 Crypto Apps" : cat === "payments" ? "📱 Payment Apps" : "💳 Credit Cards";
          return (
            <button
              key={cat}
